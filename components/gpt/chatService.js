// chatService.mjs
export async function generateChatCompletion(model,apiKey='', system, messages, displayNodeId, stream = true, responsetype = "display") {
...
async function handleResponse(response, model, displayNodeId) {
    ...
    while (isHandling) {
        ...
        isHandling = model === 'deepseek-chat' ? handleDeepSeekChatChunk(chunk, displayNode, displayTextNode) : handleOllamaChunk(chunk, displayNode);
    }
}
...
function handleOllamaChunk(chunk, displayNode) {
